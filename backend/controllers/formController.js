...
exports.updateForm = async (req, res) => {
    try {
        ...
        if (!form) {
            return res.status(404).json({ message: 'Form not found or you do not have permission to edit it.' });
        }
...
exports.getFormForEdit = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Error fetching form for editing', error: error.message });
    }
};
...
exports.submitForm = async (req, res) => {
    try {
        ...
        for (const key in req.body) {
            ...
            if (question && question.type === 'multipleAttachments' && req.body[key]) {
